...
Let's get started with the Tutor website. We will create the files, add the assets and add the boilerplate code.

Create a new folder called `tutor-website` and inside it create the following folders and files:
...
Open the project with Live Server or any other server of your choice. You should see the title "Welcome To Tutor" on the page.

## Fonts & Icons

Let's add the fonts and icons to the project. We will use `Open Sans` from Google Fonts and Font Awesome for this project.
...
We are removing any underline from all links, removing the list-style from all unordered lists and setting the width of all images to 100% of their container.

## Media Queries

Let's add media queries for our breakpoints. Add the following code to the bottom of your `styles.css` file:
...
Any CSS that we put in these media queries will only apply to screens that are less than the specified width.
